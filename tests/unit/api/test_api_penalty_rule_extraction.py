"""API tests for contract upload, penalty rule extraction, review, and publication.
...
contract create/get/publications-audit endpoints go straight to the real
...
class _FakeRuleExtractionService:
    """Double for `PenaltyRuleExtractionService`, covering the five agreed methods."""
    ...
    def create_contract(
        ...
        contracts = RetailerAgreementRepository(session)
        created = contracts.add_retailer_agreement(
    ...
    def start_extraction(self, contract_id):
        return SimpleNamespace(job_run_id=uuid4(), agent_run_id=uuid4(), thread_id=uuid4(), staged_count=2)

    def list_extracted_rules(self, contract_id, status=None):
        rows = [_fake_extracted_rule(contract_id=contract_id)]
        if status is not None:
            rows = [r for r in rows if r.status == status]
        return rows

    def submit_review(self, contract_id, extracted_rule_id, status, review_notes=None):
        return _fake_extracted_rule(
            contract_id=contract_id, id=extracted_rule_id, status=status, review_notes=review_notes
        )

    def publish(self, contract_id):
...
@pytest.fixture
def seeded_contract_id(database) -> str:
    with database.session() as session:
        contract = RetailerAgreementRepository(session).add_retailer_agreement(
            retailer_id=uuid4(),
...
def test_create_contract_then_get(client):
    created = client.post("/api/v1/penalties/contracts", json=CONTRACT_BODY)
    assert created.status_code == 201, created.text
    body = created.json()
    assert body["message"] == "Contract created."
    contract_id = body["data"]["id"]

    fetched = client.get(f"/api/v1/penalties/contracts/{contract_id}")
    assert fetched.status_code == 200
    assert fetched.json()["data"]["contract_code"] == CONTRACT_BODY["contract_code"]


def test_create_contract_idempotent_on_sha256_returns_200(client):
    first = client.post("/api/v1/penalties/contracts", json=CONTRACT_BODY)
    assert first.status_code == 201

    second_body = {**CONTRACT_BODY, "contract_code": "CONTRACT-RE-1-DUPLICATE"}
    second = client.post("/api/v1/penalties/contracts", json=second_body)
    assert second.status_code == 200, second.text
    assert second.json()["message"] == "Contract already exists."
    assert second.json()["data"]["id"] == first.json()["data"]["id"]


def test_get_contract_unknown_id_returns_404(client):
    resp = client.get(f"/api/v1/penalties/contracts/{uuid4()}")
    assert resp.status_code == 404


def test_start_extraction_returns_202(client, seeded_contract_id):
    resp = client.post(f"/api/v1/penalties/contracts/{seeded_contract_id}/extract")
    assert resp.status_code == 202, resp.text
    data = resp.json()["data"]
    assert data["contract_id"] == seeded_contract_id
    assert "agent_run_id" in data
    assert "workflow_thread_id" in data


def test_list_extracted_rules_with_status_filter(client, seeded_contract_id):
    all_rows = client.get(f"/api/v1/penalties/contracts/{seeded_contract_id}/extracted-rules")
    assert all_rows.status_code == 200
    assert len(all_rows.json()["data"]) == 1

    filtered = client.get(
        f"/api/v1/penalties/contracts/{seeded_contract_id}/extracted-rules",
...
def test_review_extracted_rule_happy_path(client, seeded_contract_id):
    extracted_rule_id = uuid4()
    resp = client.post(
        f"/api/v1/penalties/contracts/{seeded_contract_id}/extracted-rules/{extracted_rule_id}/review",
...
def test_review_extracted_rule_invalid_status_returns_422(client, seeded_contract_id):
    resp = client.post(
        f"/api/v1/penalties/contracts/{seeded_contract_id}/extracted-rules/{uuid4()}/review",
        json={"status": "MAYBE"},
    )
    assert resp.status_code == 422


def test_publish_contract_rules(client, seeded_contract_id):
    resp = client.post(f"/api/v1/penalties/contracts/{seeded_contract_id}/publish")
    assert resp.status_code == 200, resp.text
    data = resp.json()["data"]
    assert data["contract_id"] == seeded_contract_id
...
def test_publication_audit_includes_rejection_histogram(client, database, seeded_contract_id):
    ...
    with database.session() as session:
        extracted = ExtractedPenaltyRuleRepository(session).add_extracted_rule(
            contract_id=UUID(seeded_contract_id),
            agent_run_id=uuid4(),
    ...
    resp = client.get(f"/api/v1/penalties/contracts/{seeded_contract_id}/publications")
    assert resp.status_code == 200, resp.text
    data = resp.json()["data"]
    assert data["contract_id"] == seeded_contract_id
    assert len(data["outcomes"]) == 1
