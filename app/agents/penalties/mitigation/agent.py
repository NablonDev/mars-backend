...
from app.agents.penalties.mitigation.context import PenaltyMitigationSummaryContext
...
class PenaltyMitigationAgent:
    """The mitigation-summary sub-domain's LLM tool-calling loop."""
    ...
    def generate_mitigation_summary(
        self,
        ...
        *,
        ...
        try:
            for round_number in range(1, MAX_TOOL_ROUNDS):
                ...
                logger.info(
                    "Mitigation summary LLM round %s completed in %.1fs",
                    round_number,
            ...
            raise ExternalServiceError(
                ...
                details={
                    "detail": (
                        f"mitigation summary generation failed after {MAX_TOOL_ROUNDS} rounds "
        ...
        if not final_response.content:
            raise ExternalServiceError(
                ...
                details={
                    "detail": (
                        f"mitigation summary generation failed: Model returned no summary "
        ...
        if not isinstance(final_response.content, str):
            raise ExternalServiceError(
                ...
                details={
                    "detail": (
                        f"mitigation summary generation failed: Model returned non-text final "
                        f"content for order_id={order_id!r}, as_of_date={as_of_date.isoformat()}."
                    )
                },
            )

        return PenaltyMitigationSummaryOutput(
            order_id=order_id,
